...
export const mintingTokens = async ({
  ...
  // Mint tokens
  const signature = await connection.requestAirdrop(creator.publicKey, 10 * anchor.web3.LAMPORTS_PER_SOL);
...
export function createValues(defaults?: TestValuesDefaults): TestValues {
  ...
  const ammKey = PublicKey.findProgramAddressSync([id.toBuffer()], anchor.workspace.AnchorSplAmm.programId)[0];
  ...
  const poolAuthority = PublicKey.findProgramAddressSync(
    ...
    anchor.workspace.AnchorSplAmm.programId,
  )[0];
